...
| DB-02 | P2 | SQLite + WAL mode | 2026-05-13 |
| AUTH-01 | P1 | User registration endpoint | 2026-05-13 |
...
| CORSHARD | P2 | CORS hardening middleware + PICOSHOGUN_CORS_ORIGINS env var | 2026-05-29 |
...
| REBRAND | P1 | Shogun → PicoShogun rename across all surfaces | 2026-05-30 |

## Active Sprint (v2.17 — Next)
...
| CORSBLOCK-FLIP | P2 | TODO | Enforce CORS wildcard blocking in production | `block_wildcard_in_production=True` when origins are set |
| SCHEDCLEANUP | P3 | TODO | Schedule API key cleanup as periodic job | Add `cleanup_expired_keys` to scheduler as cron job |
| DISCORD-REAL | P2 | TODO | Make Discord notifier actually send messages | Plugin sends HTTP POST to Discord webhook |
| REMOVE-MASTER | P3 | TODO | Remove deprecated `orchestrator/master.py` | Delete file, update imports |
| LOADTEST | P3 | TODO | Load testing with k6 or Locust | Baseline benchmarks for rate limiting, API throughput |
| MYPYSTRICT | P3 | TODO | MyPy strict type checking | Remove `--ignore-missing-imports --no-strict-optional` |
| E2E-TESTS | P3 | TODO | Dashboard E2E tests | Playwright or Cypress tests for Command Centre SPA |
| DOCKERCI | P2 | TODO | Docker CI end-to-end with env vars | Verify Docker build works with PICOSHOGUN_* env vars |
