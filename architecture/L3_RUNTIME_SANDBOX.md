...
**SecDev Runtime Sandbox** — a lightweight process sandbox that monitors runtime behavior against a policy engine, producing deterministic verdicts (ALLOW / DENY / AUDIT) per operation. Runs locally, no cloud, no LLM.
...
- **Integration** — feeds verdicts back to SecDev AlertHub (webhook on DENY/AUDIT)
